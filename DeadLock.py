...
def check_deadlock(pn: PetriNet, reachable_bdd) -> Optional[List[int]]:
    ...
    Sigma = [pulp.LpVariable(f"Sigma_{i}", lowBound=0, cat='Integer') for i in range(num_trans)]
    ...
    for p_idx in range(num_places):
        ...
        delta_sum = pulp.lpSum([C[t_idx, p_idx] * Sigma[t_idx] for t_idx in range(num_trans)])
    ...
    for t_idx in range(num_trans):
        ...
        if not input_places:
            ...
            print(f"  [Info] Transition {pn.trans_ids[t_idx]} is a source. No deadlock possible.")
            return None
        
        # Constraint: Sum(M[p] for p in inputs) <= len(inputs) - 1
        prob += (pulp.lpSum([M[p_idx] for p_idx in input_places]) <= len(input_places) - 1), f"Disable_Trans_{t_idx}"
    ...
    while True:
        ...
        if status != pulp.LpStatusOptimal:
            ...
            print(f"  [Deadlock] ILP Infeasible. System is deadlock-free. (Time: {time.time() - start_time:.4f}s)")
            return None
        ...
        if is_reachable:
            ...
            cut_lhs = pulp.lpSum([M[i] for i in ones]) - pulp.lpSum([M[i] for i in zeros])
            cut_rhs = len(ones) - 1
